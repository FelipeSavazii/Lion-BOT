...
@bot.command()
async def ping(ctx):
    embed = discord.Embed(title=" ", description=f"🏓 **Minha latência é {int(bot.latency * 1000)}ms**", color=color)
...
async def uptime(ctx):
    ...
    embed = discord.Embed(title=" ", description=f"⏲️ **Estou acordado há {uptime}**", color=color)
    embed.set_footer(text=f"Executado por {ctx.author}")
    await ctx.send(embed=embed)
